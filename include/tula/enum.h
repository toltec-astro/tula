...
template <typename T>
concept EnumWithMeta = requires {
    requires tula::meta::EnumClass<T>;
    { enum_has_meta(meta_enum::type_t<T>{}) } -> std::same_as<std::true_type>;
};

namespace internal {

    /// @brief Return the mask value ( values | ...) of enum type.
    template <tula::meta::EnumClass T> constexpr auto get_mask() noexcept {
        ...
        } else if constexpr (constexpr auto value =
    ...
    template <tula::meta::EnumClass T> constexpr auto bitcount(T v) {
...
template <auto v>
requires tula::meta::EnumClass<decltype(v)>
...
template <tula::meta::EnumClass T> constexpr auto values() {
    using meta_t = decltype(enum_meta_type(meta_enum::type_t<T>{}));
...
template <tula::meta::EnumClass T> constexpr auto names() {
    using meta_t = decltype(enum_meta_type(meta_enum::type_t<T>{}));
    constexpr std::size_t n = meta_t::members.size();
...
template <tula::meta::EnumClass T> constexpr auto name(T v) {
    using meta_t = decltype(enum_meta_type(meta_enum::type_t<T>{}));
