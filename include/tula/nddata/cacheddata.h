...
struct NDData {
    ...
    auto derived() const -> const auto & {
        return static_cast<const Derived &>(*this);
    }
    auto derived() -> auto & { return static_cast<Derived &>(*this); }
    ...
    template <typename U = Derived>
    requires requires(U &&u) { u.data; }
    auto operator()() -> auto & { return derived().data; }
};
...
struct CachedData : NDData<CachedData<DataType, Evaluator>> {
    ...
    template <typename T>
    const auto &operator()(const T &parent) const {
        static_assert(
...
template <typename Derived>
struct LabeledData : NDData<Derived> {
