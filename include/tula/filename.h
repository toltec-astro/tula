...
std::string parse_pattern(const std::string &pattern,
    ...
    SPDLOG_TRACE(
        fmt::format("filename components: {stem}", fmt::arg("stem", stem)));
    auto parsed = fmt::format(fmt::runtime(pattern), fmt::arg("stem", stem), std::forward<decltype(args)>(args)...);
